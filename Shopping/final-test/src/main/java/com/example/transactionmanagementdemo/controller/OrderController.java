...
public class OrderController {
    ...
    @GetMapping("/user/OrderProductDetail")
    ...
    @PutMapping("/seller/complete")
    // 4. Order a. The seller should be able to complete a “Processing” order by updating its status to “Completed”.
    public String completedOrder(@RequestParam int order_id, @RequestParam int seller_id) throws Exception {
        orderService.completedOrder(order_id,seller_id);
        return "complete succeed";
    }

    @GetMapping("/seller/mostSold3")
    ...
    public List<String> getTop3MostFreq(@RequestParam Integer user_id) throws Exception {
            List<String> ans = orderService.getTop3MostFreq(user_id);
            return ans;
    }

    @GetMapping("/seller/totalItemSold")
